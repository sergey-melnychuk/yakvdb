    ...
    loop {
        ...
        match readline {
                        ...
                        match file.above(&start_key) {
                            Ok(Some(mut current_key)) => {
                                ...
                                if let Ok(Some(value)) = file.lookup(&current_key) {
                                    match mode {
                                        Mode::Hex => println!("0x{} : 0x{}", hex::encode(&current_key), hex::encode(&value)),
                                        Mode::Str => println!("\"{}\" : \"{}\"", String::from_utf8_lossy(&current_key), String::from_utf8_lossy(&value)),
                                    }
                                        ...
                                        match file.above(&current_key) {
                                            Ok(Some(next_key)) => {
                                                if let Ok(Some(value)) = file.lookup(&next_key) {
                                                    match mode {
                                                        Mode::Hex => println!("0x{} : 0x{}", hex::encode(&next_key), hex::encode(&value)),
                                                        Mode::Str => println!("\"{}\" : \"{}\"", String::from_utf8_lossy(&next_key), String::from_utf8_lossy(&value)),
                                                    }
                                                    current_key = next_key;
                                                } else {
                                                    break;
                        ...
                        match file.min() {
                            Ok(Some(mut current_key)) => {
                                ...
                                for (key, value) in entries.iter().skip(start_idx) {
                                    match mode {
                                        Mode::Hex => println!("0x{} : 0x{}", hex::encode(key), hex::encode(value)),
                                        Mode::Str => println!("\"{}\" : \"{}\"", String::from_utf8_lossy(key), String::from_utf8_lossy(value)),
                                    }
                                }
            ...
            "tree" => {                
                ...
                let total_pages = ((file_len - head_size) / page_size) as u32;
                
                // Collect all output lines for pagination
                let mut output_lines = Vec::new();
                output_lines.push("B-Tree Structure:".to_string());
                output_lines.push("=================".to_string());
                output_lines.push(format!("{:<6} {:<8} {:<6} {:<20}", "ID", "TYPE", "FILL%", "KEYS"));
                    ...
                    if let Some(page) = file.page(id) {
                        ...
                        let key_range = if page.len() > 0 {
                            let min_key = match mode {
                                ...
                                Mode::Str => format!("\"{}\"", String::from_utf8_lossy(page.min())),
                            };
                            ...
                            for (i, (key, val, page_ref)) in entries.iter().enumerate() {
                                ...
                                let val_str = if val.is_empty() {
                                    ...
                                    match mode {
                                        ...
                                        Mode::Str => format!("\"{}\"", String::from_utf8_lossy(val)),
                                    }
                                };
                                ...
                                let truncated_val = if val_str.len() > 125 {
                                    format!("{}...", &val_str[..125])
                                } else {
                                    val_str
                                };
            ...
            "root" => {
                ...
                if root_page.len() > 0 {
                    println!("\nEntries:");
                    println!("{:<4} {:<20} {:<20} {:<8}", "IDX", "KEY", "VALUE", "PAGE_REF");
                    println!("{:-<55}", "");
